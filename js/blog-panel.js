...
class BlogPanel extends HTMLElement {
  ...
  #init() {
    ...
    function updateListFooterAndSentinel(yearStr, total, toShow, sliceForDisplay) {
      if (loadSentinel) {
        const needMore =
    ...
    window.__openBlogPostByHandle = async function openBlogPostFromExternal(handle) {
      const blogDropdown = document.getElementById("blog");
      if (
    ...
    (async () => {
      ...
      if (yearFilterEl && blogPostListItems.length && minY != null && maxY != null) {
